...
(function () {
	...
	var _Phone = {
		...
		set_talk_header: function (args) { // 聊天界面头部
			...
			// 未读记录
			this.screen.set_text('微信('+this.config.wx_talk_unread+')', 30, 'white', 50, 98, {align: 'left'});
		...
		set_talk_content: function (talks) {
			...
			for(i in talks) {
				...
				switch(t.type) {
					case 'time': // 聊天时间
						this.screen.draw_round_rect('#cecece', '#cecece', bg_width / 2 - 50, top, 100, 40, 10);
						this.screen.set_text(t.content, 25, '#fff', bg_width / 2, top + 30);
						top += 70;
						break;
					case 'text': // 文本
						var fontsize = 30;
					...
					case 'voice': // 语音
						...
						if('left' == t.align) {
							...
							var w = left + len + 20;
							this.screen.set_text(t.duration + "''", fontsize, '#9B9B9B', w, top + 55, {align:'left'});
						} else {
							// 底色
							...
							// 时长
							var w = left - 20;
							this.screen.set_text(t.duration + "''", fontsize, '#9B9B9B', w, top + 55, {align:'right'});
						}
					...
					case 'video': // 视频
						...
						var len = this.screen.set_text('通话时长 ' + t.duration, fontsize, '#000', 0, 0, {max_width: bg_width, calc_only:true}).width;
						if('left' == t.align) {
							// 背景
							var left = padding * 2 + avatar_side;
							this.screen.draw_round_rect(white_border_color, white_bg_color, left, top, len + pic_width + padding * 2, avatar_side, 5);
							// 图标
							var w = left;
							self.screen.set_image('/static/imgs/phone/wx-video2.png', w, top + 10);
							// 时长
							var w = left + pic_width;
							this.screen.set_text('通话时长 ' + t.duration, fontsize, '#000', w, top + 50, {align:'left', max_width: bg_width});
						} else {
							// 背景
							...
							this.screen.set_text('通话时长 ' + t.duration, fontsize, '#000', w, top + 50, {max_width: bg_width});
					...
					case 'pay':
						...
						if('left' == t.align) {
							...
							// 金额
							this.screen.set_text('￥' + t.money, fontsize, '#FFF', left + 112, top + 100, {align:'left'});
							...
							// 金额
							this.screen.set_text('￥' + t.money, fontsize, '#FFF', left + 105, top + 100, {align:'left'});
